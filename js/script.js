...
addEvent(document, 'mouseup', function (e) {
    ...
    if (e.target.className == 'close-btn') {
        ...
        if (textArea.value !== '') {
            ...
            undoArea.addEventListener('click', undo);
            undoArea.addEventListener('click', deleteUndo);
        }
...
function undo(e) {
    ...
    if (e.target.className == 'undo-wrapper') {

        // Undo Note Pad
        var undoArea = e.target;
        var dataId = undoArea.getAttribute('data-id');
        var d = JSON.parse(localStorage.getItem(dataId));
        createNote(d.top, d.left, d.width, d.height, d.text);

        document.querySelector('body').removeChild(undoArea); // remove undo area
    }
}

function deleteUndo(e) {
...
function createNote(top, left, w, h, text) {
    ...
    note.setAttribute('id', 'post-it-' + list.length);
